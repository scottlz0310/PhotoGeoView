...
### 9.4 実行形態の違い（VS Code拡張 / CLI）

- **VS Code拡張**: エディタ中心の作業になりやすく、lint/test を回さずに終了しがち
- **cli**: ターミナル操作と相性が良く、lint/typecheck/test を実行しやすい
- **運用の前提**: どちらでも品質チェックはセッション終了前に必ず `format → lint → typecheck → test` を実行する
