...
- [ ] TailwindCSS v4 インストール
  ```bash
  pnpm add -D tailwindcss@next @tailwindcss/typography tailwindcss-animate
  ```
- [ ] `tailwind.config.ts` 作成
- [ ] PostCSS設定
- [ ] グローバルCSS更新（Tailwindディレクティブ追加）
- [ ] 動作確認（簡単なスタイル適用）
...
### 次回セッション時のTODO
1. Phase 0.2 完了（開発環境確認）
2. Phase 1.1.1 開始（TailwindCSS v4セットアップ）
