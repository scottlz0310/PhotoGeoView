...
- [ ] 地図タイル切替機能 <!-- Phase 3に移動 -->
  - [x] OpenStreetMap
  - [ ] Google Maps (API未使用、地図タイルは不採用)
  - [ ] Satellite (Mapbox Satellite を第一候補、Esri World Imageryは代替)
  - [ ] Google Maps への外部リンク追加 (ピンのPopupから遷移)
...
- [ ] サムネイルキャッシュ
  ...
  - [ ] メモリキャッシュ (フロントエンド側)
- [ ] 仮想スクロールの最適化
- [ ] 地図タイルキャッシュ戦略 (WebView2制約の検証含む) <!-- 2025-12-31提案: WinUI3でWebView2キャッシュ問題→Mapsui移行の経緯を踏まえる -->
  - [ ] WebView2でのタイルキャッシュ挙動の技術検証 (Windows)
  - [ ] タイル提供元の利用規約確認 (OSM/Google/衛星)
  - [ ] 依存しないキャッシュ方式の設計 (Tauri FS / IndexedDB)
  - [ ] レート制限対策の方針決定 (キャッシュ優先/代替タイル/自己ホスト)
  - [ ] FSキャッシュプロトタイプ (Tauri FS)
  - [ ] タイルキャッシュの保存形式・容量上限・TTL設計
  - [ ] キャッシュの失効/削除戦略 (LRU/期間)
  - [ ] 代替案の検討: ネイティブ描画 (Tauri + Rust GISクレート)
  - [ ] 実装優先順: 規約確認 → FSキャッシュプロトタイプ → TTL/LRU
  - [ ] 基盤タイルはOSM Standardを前提に規約完全準拠
  - [ ] 衛星レイヤーはMapbox優先、Esriは代替 (ユーザー設定でオン/オフ切替)
  - [ ] GoogleはAPI不使用 (外部リンクのみで導線確保)
  - [ ] Google Mapsリンクは写真の緯度経度・ズームをそのまま使用 (例: 18→18, URL形式: https://www.google.com/maps/@lat,lng,18z)
  - [ ] Mapbox Satellite 利用条件の反映 (TTL 12h・CDNキャッシュ、エンドユーザー単位キャッシュ、オフライン大量NG、APIキー必須、無料枠/有料プラン、目安: 6000 req/min、Leaflet相性良好)
