...
from src.integration.services.file_discovery_errors import (
    FileDiscoveryError, FolderAccessError, FolderNotFoundError,
    PermissionDeniedError, FileValidationError, CorruptedFileError
)
...
class FileListDisplayErrorHandlingTest(unittest.TestCase):
    ...
    def test_01_nonexistent_folder_error_handling(self):
        ...
        try:
            ...
            self.assertEqual(len(discovered_images), 0, "存在しないフォルダでは画像が検出されない必要があります")
            ...
            self.assertTrue(japanese_log_found, "日本語のエラーメッセージがログに記録される必要があります")
    ...
    def test_02_permission_denied_error_handling(self):
        ...
        try:
            ...
            if os.name != 'nt':  # Linux/Unix環境
                self.assertEqual(len(discovered_images), 0, "権限のないフォルダでは画像が検出されない必要があります")
            ...
            if os.name != 'nt':
                self.assertTrue(permission_log_found, "権限エラーの日本語メッセージがログに記録される必要があります")
            ...
            test_result = {
                ...
                "os_type": os.name,
                "log_messages": [msg["message"] for msg in self.log_messages if any(k in msg["message"] for k in ["権限", "アクセス", "Permission"])],
    ...
    def test_03_corrupted_files_error_handling(self):
        ...
        try:
            ...
            # 結果の検証
            self.assertIsInstance(discovered_images, list, "戻り値はリスト型である必要があります")
            self.assertEqual(len(discovered_images), 0, "破損ファイルのみのフォルダでは有効な画像が検出されない必要があります")
            ...
            self.assertTrue(corruption_log_found, "破損ファイル検出の日本語メッセージがログに記録される必要があります")
            ...
            for filename, is_valid in validation_results.items():
                self.assertFalse(is_valid, f"破損ファイル {filename} は無効と判定される必要があります")
            ...
            test_result = {
                ...
                "log_messages": [msg["message"] for msg in self.log_messages if any(k in msg["message"] for k in ["破損", "無効", "エラー"])],
    ...
    def test_04_mixed_folder_error_handling(self):
        ...
        try:
            ...
            # 結果の検証
            self.assertIsInstance(discovered_images, list, "戻り値はリスト型である必要があります")
            self.assertEqual(len(discovered_images), 1, "正常ファイル1個のみが検出される必要があります")

            # 検出されたファイルが正常ファイルであることを確認
            if discovered_images:
                detected_file = discovered_images[0]
                self.assertEqual(detected_file.name, "正常画像.jpg", "正常ファイルが検出される必要があります")
            ...
            test_result = {
                ...
                "log_messages": [msg["message"] for msg in self.log_messages if any(k in msg["message"] for k in ["除外", "破損", "無効"])],
    ...
    def test_06_error_recovery_and_continuation(self):
        ...
        try:
            ...
            for result in processing_results:
                ...
                print(f"   {result['folder_type']}: {status_icon} (画像: {result['images_found']}個, {result['duration']:.3f}秒)")
                if result["error"]:
    ...
    def test_07_ui_error_integration(self):
        ...
        try:
            ...
            self.assertTrue(mock_folder_navigator._handle_discovery_error.called or
                          mock_folder_navigator._show_no_images_message.called,
            ...
            for result in ui_error_results:
                print(f"   {Path(result['folder_path']).name}: {result['expected_error']} (画像: {result['images_found']}個)")
    ...
    def generate_error_handling_report(self) -> Dict[str, Any]:
        ...
        japanese_success_rate = len([r for r in japanese_message_tests if r["status"] == "passed"]) / len(japanese_message_tests) if japanese_message_tests else 0
...
def run_error_handling_tests():
    ...
    try:
        # 各テストを実行してレポートデータを収集
        for test_method in ['test_01_nonexistent_folder_error_handling',
