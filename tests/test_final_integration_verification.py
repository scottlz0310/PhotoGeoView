...
class FinalIntegrationVerificationTest(unittest.TestCase):
    ...
    def test_01_complete_workflow_integration(self):
        ...
        try:
            ...
            self.assertTrue(watch_status.get('is_watching', False), "ファイルシステム監視が開始されている必要があります")
    ...
    def test_03_japanese_localization_final(self):
        ...
        try:
            ...
            self.assertGreater(len(japanese_images), 0, "日本語ファイル名の画像が検出される必要があります")

            # 2. 日本語ファイル名の検証
            japanese_names = [img.name for img in japanese_images]
            expected_japanese_names = ["風景写真.jpg", "ポートレート.png", "スクリーンショット.gif", "テスト画像.bmp"]

            for expected_name in expected_japanese_names:
                self.assertIn(expected_name, japanese_names, f"日本語ファイル名が正しく処理される必要があります: {expected_name}")
            ...
            self.assertEqual(sub_images[0].name, "サブ画像.jpg", "日本語サブフォルダの画像名が正しく処理される必要があります")
        ...
        try:
            ...
            for check_name, check_data in performance_checks.items():
                ...
                if "duration" in check_data:
                    print(f"   {status} {check_data['description']}: {check_data['duration']:.3f}秒 (閾値: {check_data['threshold']}秒)")
                elif "usage_mb" in check_data:
                    print(f"   {status} {check_data['description']}: {check_data['usage_mb']:.1f}MB (閾値: {check_data['threshold']}MB)")
                elif "time" in check_data:
                    print(f"   {status} {check_data['description']}: {check_data['time']:.3f}秒 (閾値: {check_data['threshold']}秒)")
    ...
    def test_05_file_system_watcher_integration(self):
        ...
        try:
            ...
            self.assertEqual(final_images[0].name, "新しい画像.png", "新しい画像が検出される必要があります")
