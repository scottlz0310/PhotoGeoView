...
class FileListDisplayIntegrationTest(unittest.TestCase):
    ...
    def test_01_folder_selection_to_thumbnail_display_normal_case(self):
        ...
        try:
            ...
            for image_path in discovered_images:
                self.assertIsInstance(image_path, Path, "検出されたファイルはPathオブジェクトである必要があります")
                self.assertTrue(image_path.exists(), f"検出されたファイルが存在する必要があります: {image_path}")
                self.assertIn(image_path.suffix.lower(), expected_extensions,
                            f"対応する画像形式である必要があります: {image_path.suffix}")

            # 4. 無効なファイルが除外されていることを確認
            discovered_names = [p.name for p in discovered_images]
            self.assertNotIn("document.txt", discovered_names, "テキストファイルは除外される必要があります")
            self.assertNotIn("video.mp4", discovered_names, "動画ファイルは除外される必要があります")
            self.assertNotIn("corrupted.jpg", discovered_names, "破損ファイルは除外される必要があります")
    ...
    def test_02_empty_folder_handling(self):
        ...
        try:
            ...
            # 2. 結果の検証
            self.assertIsInstance(discovered_images, list, "検出結果はリスト型である必要があります")
            self.assertEqual(len(discovered_images), 0, "空フォルダでは画像が検出されない必要があります")
        ...
        try:
            ...
            for file_name, is_valid in validation_results.items():
                ...
                if file_path.suffix.lower() in valid_extensions and not file_name.startswith('corrupted'):
                    self.assertTrue(is_valid, f"有効な画像ファイルがバリデーションを通過する必要があります: {file_name}")
                elif file_name.startswith('corrupted'):
                    self.assertFalse(is_valid, f"破損ファイルはバリデーションで除外される必要があります: {file_name}")
                elif file_path.suffix.lower() not in valid_extensions:
                    self.assertFalse(is_valid, f"無効な拡張子のファイルはバリデーションで除外される必要があります: {file_name}")
    ...
    def test_06_japanese_message_display(self):
        ...
        try:
            ...
            self.assertEqual(len(discovered_images_nonexistent), 0, "存在しないフォルダでは画像が検出されない")
    ...
    def test_07_performance_basic_check(self):
        ...
        try:
            ...
            self.assertLess(avg_duration, max_acceptable_duration,
