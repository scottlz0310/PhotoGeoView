...
class PerformanceBenchmarkSuite:
    ...
    def run_all_benchmarks(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"パフォーマンスベンチマークが完了しました (所要時間: {total_duration:.2f}秒)")
            return summary
    ...
    def benchmark_thumbnail_generation(self) -> Dict[str, Any]:
        ...
        for impl in implementations:
            ...
            for i in range(5):  # 5回実行
                result = self._run_thumbnail_generation_test(impl, self.test_images[:50])
                impl_results.append(result)
                self.results.append(result)
    ...
    def benchmark_ui_responsiveness(self) -> Dict[str, Any]:
        ...
        for impl in implementations:
            ...
            results[impl] = {
                ...
                "frame_rate": 1.0 / statistics.mean(durations) if statistics.mean(durations) > 0 else 0
            }
    ...
    def benchmark_memory_efficiency(self) -> Dict[str, Any]:
        ...
        for impl in implementations:
            ...
            results[impl] = {
                ...
                "memory_efficiency": result.memory_average / result.memory_peak if result.memory_peak > 0 else 0
            }
    ...
    def _run_image_loading_test(self, implementation: str, images: List[Path]) -> BenchmarkResult:
        """画像読み込みテストの実行"""
        start_time = time.time()
    ...
    def _run_thumbnail_generation_test(self, implementation: str, images: List[Path]) -> BenchmarkResult:
        """サムネイル生成テストの実行"""
        start_time = time.time()
    ...
    def _run_exif_parsing_test(self, implementation: str, images: List[Path]) -> BenchmarkResult:
        """EXIF解析テストの実行"""
        start_time = time.time()
    ...
    def _generate_comparison_summary(self) -> Dict[str, Any]:
        ...
        if "integrated" in summary and "cursor_only" in summary:
            ...
            summary["performance_improvement"] = {
                "duration_improvement": (cursor_duration - integrated_duration) / cursor_duration * 100,
                "memory_efficiency": summary["integrated"]["average_memory_peak"] / summary["cursor_only"]["average_memory_peak"]
            }
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if memory_results:
            recommendations.append("メモリ使用量が高い処理があります。キャッシュ戦略の見直しを推奨します。")

        # CPU使用量の分析
        cpu_results = [r for r in self.results if r.cpu_peak > 70]
        if cpu_results:
            recommendations.append("CPU使用率が高い処理があります。並行処理の最適化を推奨します。")

        # 応答時間の分析
        slow_results = [r for r in self.results if r.duration > 1.0]
        if slow_results:
            recommendations.append("応答時間が遅い処理があります。アルゴリズムの最適化を推奨します。")

        if not recommendations:
            recommendations.append("パフォーマンスは良好です。現在の実装を維持してください。")
