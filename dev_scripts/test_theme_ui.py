...
class ThemeTestWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.config_manager = ConfigManager(
            config_dir=Path("config"), logger_system=self.logger_system
        )
        ...
        self.theme_manager = IntegratedThemeManager(
            self.config_manager, state_manager, self.logger_system
        )
    ...
    def populate_theme_combo(self):
        ...
        for theme_name in sorted(available_themes):
            ...
            display_name = (
                theme_config.get("display_name", theme_name.replace("_", " ").title())
                if theme_config
        ...
        if text:
            ...
            if theme_name:
                ...
                if theme_config:
                    ...
                    info += (
                        f"Primary Color: {theme_config.get('primaryColor', 'N/A')}\n"
                    )
                    info += (
                        f"Background: {theme_config.get('backgroundColor', 'N/A')}\n"
                    )
