...
class TestBasicImports(unittest.TestCase):
    ...
    def test_project_structure(self):
        ...
        self.assertTrue(
            (project_root / "pyproject.toml").exists(), "pyproject.toml should exist"
        )

        # 重要なディレクトリの存在確認
        self.assertTrue((project_root / "src").exists(), "src directory should exist")
        self.assertTrue(
            (project_root / "tests").exists(), "tests directory should exist"
        )
...
class TestEnvironment(unittest.TestCase):
    ...
    def test_working_directory(self):
        ...
        # プロジェクトルートにいることを確認
        self.assertTrue(
            (cwd / "main.py").exists() or (cwd / "pyproject.toml").exists(),
