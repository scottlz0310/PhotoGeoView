...
class ThemeTestWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # サンプルラベル
        sample_label = QLabel(
            "これはサンプルテキストです。テーマが適用されると色が変わります。"
        )
        sample_label.setStyleSheet(
            "font-size: 14px; padding: 10px; border: 1px solid gray; margin: 5px;"
        )
        ...
        try:
            ...
            if success:
                ...
                if theme_config:
                    ...
                    self.log_display.append(
                        f"   背景色: {colors.get('background', 'N/A')}"
                    )
                    self.log_display.append(
                        f"   テキスト色: {colors.get('foreground', 'N/A')}"
                    )
                    self.log_display.append(
                        f"   プライマリ色: {colors.get('primary', 'N/A')}"
                    )
