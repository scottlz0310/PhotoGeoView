...
try:
    ...
    from .models import CheckTask, SimulationResult, CheckStatus
...
class CISimulator:
    ...
    def _run_checks(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            available_tasks = self.orchestrator.filter_tasks_by_selection(
                tasks,
                [task.check_type for task in tasks]
            )

            if len(available_tasks) < len(tasks):
                unavailable_count = len(tasks) - len(available_tasks)
                self.logger.warning(f"{unavailable_count} tasks skipped due to unavailable checkers")
            ...
            simulation_result = SimulationResult(
                ...
                python_versions_tested=list(set(
                    task.python_version for task in available_tasks
    ...
    def _handle_hook_command(self, args: argparse.Namespace) -> int:
        ...
        try:
            if not hasattr(args, 'hook_action') or args.hook_action is None:
                self.logger.error("No hook action specified. Use 'hook --help' for available actions.")
                return 1
        ...
        try:
            ...
            if args.format == 'json':
                ...
                for hook_type, info in hooks_info.items():
                    status_icon = "✅" if info['installed'] and info['is_ours'] else "❌"
            ...
            if status['hooks']:
                ...
                for hook_type, info in status['hooks'].items():
                    ...
                    if info['installed']:
                        ...
                        print(f"  Executable: {info['executable']}")
                        if info['config']:
                            print(f"  Installed At: {info['config'].get('installed_at', 'unknown')}")
                            checks = info['config'].get('checks', [])
                            print(f"  Checks: {', '.join(checks) if checks else 'none'}")
                    print()
    ...
    def _generate_summary(self, check_results: Dict[str, Any]) -> str:
        ...
        successful = sum(1 for result in check_results.values() if result.status == CheckStatus.SUCCESS)
        failed = sum(1 for result in check_results.values() if result.status == CheckStatus.FAILURE)
        warnings = sum(1 for result in check_results.values() if result.status == CheckStatus.WARNING)
        skipped = sum(1 for result in check_results.values() if result.status == CheckStatus.SKIPPED)
    ...
    def _generate_reports(self, result: SimulationResult, args: argparse.Namespace) -> Dict[str, str]:
        """Generate reports in requested formats."""
        report_paths = {}

        # Determine output directory
        output_dir = Path(args.output_dir) if args.output_dir else Path(self.config['output_directory'])
    ...
    def _generate_basic_report(self, result: SimulationResult, output_path: Path) -> None:
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            for name, check_result in result.check_results.items():
                f.write(f"{name}: {check_result.status.value} ({check_result.duration:.2f}s)\n")
    ...
    def _save_execution_history(self, result: SimulationResult) -> None:
        ...
        try:
            history_dir = Path(self.config.get('directories', {}).get('history', '.kiro/ci-history'))
    ...
    def setup_git_hook(self, hook_type: str = 'pre-commit', checks: Optional[List[str]] = None) -> bool:
        """
    ...
    def cleanup(self, keep_reports: bool = True) -> None:
        ...
        try:
            ...
            temp_dir = Path(self.config.get('directories', {}).get('temp', 'temp/ci-simulation'))
            if temp_dir.exists():
            ...
            if not keep_reports:
                reports_dir = Path(self.config.get('directories', {}).get('reports', 'reports/ci-simulation'))
    ...
    def interactive_mode(self) -> int:
        ...
        try:
            ...
            while True:
                try:
                    choice = input("\nSelect checks to run (comma-separated numbers): ").strip()
                    if choice == '0':
                        return 0
                    ...
                    selected_checks = [available_checks[c-1] for c in choices if 1 <= c <= len(available_checks)]
